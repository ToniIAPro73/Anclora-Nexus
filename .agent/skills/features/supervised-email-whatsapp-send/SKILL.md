...
4) sdd/features/supervised-email-whatsapp-send/supervised-email-whatsapp-send-spec-v1_2.md
5) .agent/rules/feature-supervised-email-whatsapp-send.md
...
- Si SMTP está configurado, permitir `native_email` solo por acción explícita de la persona.
- Aplicar encoding seguro en las URLs de lanzamiento.
- Registrar la interacción primero como `programado`.
- Exigir confirmación humana para pasar la interacción a `realizado`, salvo `native_email` disparado por la persona desde el workbench.
- Persistir el `transport` usado y devolver feedback visible al workbench.
