...
export default function PropertiesPage() {
  ...
  const formatRelativeLastUpdate = (raw?: string) => {
    ...
    const rtf = new Intl.RelativeTimeFormat(localeMap[language] || "es-ES", {
      numeric: "always",
    });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {properties.length === 0 ? (
              <div className="col-span-full bg-navy-surface/40 border border-soft-subtle rounded-2xl p-12 text-center">
                <p className="text-soft-muted italic">
                  {t("noPipelineProperties")}
                </p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-3">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <h3
                                className="text-lg font-bold text-soft-white group-hover:text-gold transition-colors leading-tight line-clamp-1"
                      ...
                      <div className="p-6 space-y-4 flex-1">
                        ...
                        {isProspected ? (
                          <>
                            ...
                            {pbmMetaByPropertyId[property.id]?.bestMatchScore ||
                              ...
                              <div className="flex items-center justify-between pt-2">
                                ...
                                <div className="flex items-center gap-2">
                                  <div className="h-1.5 w-16 bg-navy-deep rounded-full overflow-hidden">
                                    <div
                                      className="h-full bg-linear-to-r from-gold to-amber-500"
                                      style={{
                                        width: `${pbmMetaByPropertyId[property.id]?.bestMatchScore || property.match_score}%`,
                                      }}
                                    />
                                  </div>
                                  <span className="text-xs font-bold text-gold">
                                    {pbmMetaByPropertyId[property.id]
                                      ?.bestMatchScore || property.match_score}
                                    %
                                  </span>
                                </div>
                              </div>
                      ...
                      <div className="p-4 bg-navy-deep/30 border-t border-soft-subtle/30 flex justify-between items-center">
                        <span className="text-xs text-soft-muted italic">
                          {formatRelativeLastUpdate(property.last_update)}
                        </span>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={(e) => {
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 bg-navy-surface/40 border border-soft-subtle rounded-xl">
              ...
              <span className="text-xs text-soft-muted font-medium">
                {t("pageLabel")}{" "}
                <span className="text-gold font-bold">{safeCurrentPage}</span>{" "}
