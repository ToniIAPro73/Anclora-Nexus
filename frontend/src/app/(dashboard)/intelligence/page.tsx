  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-700">
        ...
        <div>
           <div className="flex items-center gap-3 mb-1">
            ...
            <div className="px-2 py-0.5 rounded-full bg-gold/10 border border-gold/30 flex items-center gap-1">
              ...
              <span className="text-[10px] font-bold text-gold uppercase tracking-tighter">Verified Engine</span>
            </div>
          </div>
          <p className="text-soft-muted max-w-2xl">
            {t('intelligenceSubtitle')}
          </p>
        </div>

        <div className="flex items-center gap-4 bg-navy-surface/50 backdrop-blur-md border border-white/5 rounded-2xl p-4">
          <div className="text-right">
            <p className="text-[10px] font-bold uppercase tracking-widest text-soft-muted">Governance Status</p>
            <p className="text-xs text-emerald-400 font-bold flex items-center gap-1 justify-end">
              <Lock className="w-3 h-3" /> Immutable Audit Active
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 h-[calc(100vh-14rem)] sticky top-28">
           <ChatConsole />
        </div>

        {/* RIGHT COLUMN: ANALYSIS & DECISION (approx. 7/12) */}
        <div className="lg:col-span-7 space-y-8 min-h-[calc(100vh-14rem)]">
          <AnimatePresence mode="wait">
            {!latestResponse ? (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="h-full border-2 border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center p-20 text-center opacity-30 min-h-[500px]"
              >
                <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4">
                  <Brain className="w-8 h-8 text-soft-muted" />
                </div>
                <p className="text-sm italic">Waiting for strategic query entry...</p>
              </motion.div>
            ) : (
              <motion.div 
                key="results"
              ...
              >
                ...
                <div className="p-4 rounded-xl bg-navy-darker/40 border border-white/5 flex items-center justify-between text-[10px] font-mono text-soft-muted">
                   <div className="flex items-center gap-4">
                     <span>AUDIT_ID: {latestResponse.audit_id || 'LOCAL_EXE'}</span>
