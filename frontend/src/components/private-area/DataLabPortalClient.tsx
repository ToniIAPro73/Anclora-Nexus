            ...
            {requestSuccess ? (
              ...
              <form className="mt-4 space-y-3" onSubmit={handleSubmit}>
                <div className="grid gap-3 md:grid-cols-2">
                  <input className="ui-input" placeholder={t('privateAreaDataLabFieldFullName')} value={form.full_name} onChange={(e) => updateForm('full_name', e.target.value)} autoComplete="name" required disabled={requestLoading} />
                  <input className="ui-input" type="email" placeholder={t('privateAreaDataLabFieldEmail')} value={form.email} onChange={(e) => updateForm('email', e.target.value)} autoComplete="email" required disabled={requestLoading} />
                  <input className="ui-input" placeholder={t('privateAreaDataLabFieldCompany')} value={form.company_name} onChange={(e) => updateForm('company_name', e.target.value)} autoComplete="organization" disabled={requestLoading} />
                  <input className="ui-input" placeholder={t('privateAreaDataLabFieldWebsite')} value={form.website_url} onChange={(e) => updateForm('website_url', e.target.value)} inputMode="url" disabled={requestLoading} />
                </div>
                <div className="grid gap-3 md:grid-cols-2">
                  <select className="ui-select" value={form.profile_type} onChange={(e) => updateForm('profile_type', e.target.value as DataLabProfileType)} disabled={requestLoading}>
                    {(['partner', 'client', 'investor', 'other'] as const).map((item) => (
                      <option key={item} value={item}>{t(`dataLabAccessProfile_${item}` as never)}</option>
                    ))}
                  </select>
                  <select className="ui-select" value={form.requested_scope} onChange={(e) => updateForm('requested_scope', e.target.value as DataLabScope)} disabled={requestLoading}>
                ...
                <textarea className="ui-textarea min-h-28" placeholder={t('privateAreaDataLabFieldIntendedUse')} value={form.intended_use} onChange={(e) => updateForm('intended_use', e.target.value)} required disabled={requestLoading} />
                <div className="grid gap-3 md:grid-cols-2">
                  <input className="ui-input" placeholder={t('privateAreaDataLabFieldGeography')} value={form.geography_focus} onChange={(e) => updateForm('geography_focus', e.target.value)} disabled={requestLoading} />
                  <input className="ui-input" placeholder={t('privateAreaDataLabFieldLanguages')} value={form.languages} onChange={(e) => updateForm('languages', e.target.value)} disabled={requestLoading} />
                </div>
                <textarea className="ui-textarea min-h-24" placeholder={t('privateAreaDataLabFieldNotes')} value={form.notes} onChange={(e) => updateForm('notes', e.target.value)} disabled={requestLoading} />
                <RecaptchaPanel token={captchaToken} onTokenChange={setCaptchaToken} />
                <label className="ui-checkbox-row">
                  <input className="ui-checkbox" type="checkbox" checked={form.newsletter_opt_in} onChange={(e) => updateForm('newsletter_opt_in', e.target.checked)} disabled={requestLoading} />
                  {t('externalFormNewsletterOptIn')}
                </label>
                <label className="ui-checkbox-row items-start">
                  <input className="ui-checkbox mt-1" type="checkbox" checked={form.privacy_accepted} onChange={(e) => updateForm('privacy_accepted', e.target.checked)} disabled={requestLoading} />
                  <span>
                ...
                {requestError ? <p className="rounded-2xl border border-rose-400/30 bg-rose-950/20 px-4 py-3 text-sm text-rose-200" role="alert" aria-live="polite">{requestError}</p> : null}
