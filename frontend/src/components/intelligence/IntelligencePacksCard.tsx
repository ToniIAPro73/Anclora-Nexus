...
export function IntelligencePacksCard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("intelligencePacksLoadingError"),
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("intelligencePacksActivateError"),
      );
    } finally {
      setSaving(false);
    }
  ...
  const createPack = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("intelligencePacksCreateError"),
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section className="surface-primary surface-copy-safe rounded-2xl border border-soft-subtle/30 bg-navy-surface/40 p-5 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-semibold text-soft-white">
                {t("intelligencePacksTitle")}
              </h2>
              <p className="text-sm text-soft-muted">
                {t("intelligencePacksSubtitle")}
              </p>
            </div>
          </div>
          <p className="text-xs text-soft-muted mt-3 wrap-break-word">
            {t("intelligencePacksIsolation")}
          </p>
        </div>
        <button
          type="button"
      ...
      {error && (
        <div className="surface-secondary rounded-xl border border-red-400/25 bg-red-500/5 p-3 mt-4 text-sm text-red-200 wrap-break-word">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.4fr_1fr] gap-4 mt-5">
        <div className="surface-secondary surface-copy-safe rounded-xl border border-soft-subtle/20 bg-navy-darker/20 p-4 min-w-0">
          <div className="flex items-center justify-between gap-3 mb-4">
            <h3 className="section-title text-soft-white">
              {t("intelligencePacksTitle")}
            </h3>
            ...
            <div className="space-y-3">
              {packs.map((pack) => {
                const lastSyncedLabel = pack.last_synced_at
                  ? new Date(pack.last_synced_at).toLocaleString(
                ...
                return (
                  <article
                    key={pack.id}
                    className="surface-secondary surface-copy-safe rounded-xl border border-soft-subtle/30 bg-navy-deep/30 p-4 min-w-0"
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <h4 className="text-base font-semibold text-soft-white wrap-break-word">
                            {pack.pack_label}
                          </h4>
                          {pack.is_default && (
                        ...
                        <p className="text-sm text-soft-muted wrap-break-word mt-1">
                          {pack.notebook_name}
                        </p>
                      </div>
                      <button
                        type="button"
                        disabled={
                          saving || activePackId === pack.id || pack.synthetic
                        }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 text-sm">
                      <div>
                        <p className="kpi-label">
                          {t("intelligencePacksNotebook")}
                        </p>
                        <p className="text-soft-white break-all mt-1">
                          {pack.notebook_id}
                        </p>
                      </div>
                      <div>
                        <p className="kpi-label">
                          {t("intelligencePacksZones")}
                        </p>
                        <p className="text-soft-white wrap-break-word mt-1">
                          {(pack.zone_scope || []).join(", ") || "—"}
                        </p>
                      </div>
                      <div>
                        <p className="kpi-label">
                          {t("intelligencePacksMarketScope")}
                        </p>
                        <p className="text-soft-white mt-1">
                          {t(
                            `intelligencePackMarketScope_${pack.market_scope}` as TranslationKey,
                          )}
                        </p>
                      </div>
                      <div>
                        <p className="kpi-label">
                          {t("intelligencePacksSourceMode")}
                        </p>
                        <p className="text-soft-white mt-1">
                          {t(
                            `intelligencePackSourceMode_${pack.source_mode}` as TranslationKey,
                          )}
                        </p>
                      </div>
                      <div>
                        <p className="kpi-label">
                          {t("intelligencePacksLanguage")}
                        </p>
                        <p className="text-soft-white mt-1">
                      ...
                      <div>
                        ...
                        <p className="text-soft-white mt-1">
                          {t(
                            `intelligencePackStatus_${pack.status}` as TranslationKey,
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4 text-sm">
                      <div className="surface-secondary rounded-xl border border-soft-subtle/20 bg-navy-darker/20 p-3">
                        <p className="kpi-label">
                          {t("intelligencePacksFreshness")}
                        </p>
                        <p className="kpi-value text-soft-white mt-1">
                          {pack.age_hours == null ? "—" : `${pack.age_hours}h`}
                        </p>
                      </div>
                      <div className="surface-secondary rounded-xl border border-soft-subtle/20 bg-navy-darker/20 p-3">
                        <p className="kpi-label">
                          {t("intelligencePacksLastSynced")}
                        </p>
                        <p className="text-soft-white mt-1 wrap-break-word">
                          {lastSyncedLabel}
                        </p>
                      </div>
                      <div className="surface-secondary rounded-xl border border-soft-subtle/20 bg-navy-darker/20 p-3">
                        ...
                        <p className="text-soft-white mt-1">
                          {pack.insight_count || 0} insights ·{" "}
                          {(pack.zones_with_data || []).length} zonas
                        </p>
                      </div>
                    </div>
                  </article>
          ...
          <div className="space-y-3">
            <label className="block">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, pack_label: e.target.value }))
                }
                placeholder={t("intelligencePacksLabelPlaceholder")}
              />
            </label>
            <label className="block">
              <span className="kpi-label">
                {t("intelligencePacksNotebookName")}
              </span>
              <input
                className={inputClassName()}
                value={form.notebook_name}
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
            ...
            <label className="block">
              <span className="kpi-label">
                {t("intelligencePacksNotebookId")}
              </span>
              <input
                className={inputClassName()}
                value={form.notebook_id}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, notebook_id: e.target.value }))
                }
                placeholder="uuid-notebook"
              />
            </label>
            <label className="block">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, zone_scope: e.target.value }))
                }
                placeholder={t("intelligencePacksZonesPlaceholder")}
              />
            </label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                >
                  {["seller", "buyer", "mixed"].map((scope) => (
                    <option key={scope} value={scope}>
                      {t(
                        `intelligencePackMarketScope_${scope}` as TranslationKey,
                      )}
                    </option>
                  ))}
                </select>
              ...
              <label className="block">
                ...
                >
                  {["notebooklm_manual", "live_sync_pack", "imported_rag"].map(
                    (mode) => (
                      <option key={mode} value={mode}>
                        {t(
                          `intelligencePackSourceMode_${mode}` as TranslationKey,
                        )}
                      </option>
                    ),
                  )}
            ...
            >
              ...
              {saving
                ? t("intelligencePacksCreating")
                : t("intelligencePacksCreate")}
            </button>
