  ...
  if (candidates.length === 0) {
    return (
      <div className="glass-panel p-6 text-center border border-white/5">
        <Copy className="w-12 h-12 text-soft-white/20 mx-auto mb-4" />
        <p className="text-soft-white/60">{t('dqNoCandidatesFound')}</p>
      </div>
    )
  }
          ...
          <div key={candidate.id} className="glass-panel p-5 border border-white/5 bg-navy-surface/40">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-soft-white/40 text-xs">#{candidate.id.split('-')[0]}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-soft-white/60 text-sm">{t('dqSimilarityScore')}:</span>
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex-1 p-3 rounded-lg bg-navy-darker/50 border border-white/5 truncate">
                <span className="text-soft-white/40 text-[10px] block uppercase mb-1">{t('dqEntity')} A</span>
                <span className="text-soft-white text-sm font-medium">
                  {candidate.left_entity_id.split('-')[0]}...
                </span>
              </div>
              <ArrowRight className="w-4 h-4 text-soft-white/20 flex-shrink-0" />
              <div className="flex-1 p-3 rounded-lg bg-navy-darker/50 border border-white/5 truncate">
                <span className="text-soft-white/40 text-[10px] block uppercase mb-1">{t('dqEntity')} B</span>
                <span className="text-soft-white text-sm font-medium">
            ...
            {candidate.signals && Object.keys(candidate.signals).length > 0 && (
              <div className="mb-6 flex flex-wrap gap-2">
                {Object.entries(candidate.signals).map(([key]) => (
                  <div key={key} className="flex items-center gap-1.5 px-2 py-1 rounded bg-white/5 border border-white/5 text-[10px] text-soft-white/70">
            ...
            <div className="flex gap-3 mt-4 pt-4 border-t border-white/5">
              ...
              <button
                ...
                disabled={!!resolvingId}
                className="flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-white/5 text-soft-white/60 border border-white/5 hover:bg-white/10 hover:text-soft-white transition-all text-sm disabled:opacity-50"
              >
                <X className="w-4 h-4" />
                {t('dqReject')}
              </button>
